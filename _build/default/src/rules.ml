...
and check_equivelence pattern p q =
  ...
  | Conn (conn1, p1, q1), Conn (conn2, p2, q2) when conn1 = conn2 ->
      (check_equivelence pattern p1 p2 || check_equivelence pattern q1 q2) && (p1 = p2 || q1 = q2)
  | Not p, Not q ->
...
and check_implication pattern p1 p2 q =
  ...
  | Conn (conn1, r1, s1), Conn (conn2, r2, s2) when conn1 = conn2 ->
        (check_implication pattern r1 p2 r2 || check_implication pattern s1 p2 s2) && (r1 = r2 || s1 = s2)
  | Not r, Not s ->
